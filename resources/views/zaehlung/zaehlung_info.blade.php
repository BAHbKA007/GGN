    ...
    <ul class="list-group">
        ...
        @endif
            <li class="list-group-item">
                ...
                <span style="margin-left:10px"><span style="font-weight: bold;">{{$item->menge}} </span>{{$item->bezeichnung}} <small>( {{$item->erzeuger}} | GRASP: @if (isset($item->grasp_status)) {{$item->grasp_status}} @if (isset($item->grasp_valid_to_current)) bis {{strftime("%d.%m.%Y", strtotime($item->grasp_valid_to_current))}} @endif @else NEIN @endif)</small></span>
